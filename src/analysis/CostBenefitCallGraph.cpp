...
static llvm::cl::opt<std::string> benefits_filename(
     "Pbenefits-filename",
...
using namespace llvm;
  ...
  const Function* CostBenefitInfo::get_function() const
  { return m_func; }

  unsigned CostBenefitInfo::get_benefit() const
  { return m_benefit; }

  unsigned CostBenefitInfo::get_cost() const {
    return m_loops.size();
  }

  void CostBenefitInfo::write(llvm::raw_ostream& o) const {
    o << get_function()->getName() << " "
      << get_benefit() << " "
      << get_cost();
  }
  
  
  CostBenefitCG::CostBenefitCG(Module &M, CallGraph& CG)
    : m_M(M)
    , m_CG(CG)
  { }
  ...
  /* Helpers */
  template<typename Map>
  static unsigned get(const Map& m, const Function* fn) {
    auto it = m.find(fn);
    if (it != m.end()) {
      return it->second;
    } else {
  ...
  static void check_loop_info_consistency(LoopInfo& li, const Function* fun, unsigned line) {
    if (!li.empty()) {
      Loop* L = *li.begin();
	...
	if (LH->getParent() != fun) {
	  errs () << "Error at " << __FILE__ << "::" << line << ":  "
		  ...
		  << " does not belong to current function "
		  << fun->getName() << "\n";
	}
    	...
    	if (const Function* callee = CS.getCalledFunction()) {
    	  ...
    	  if (!stack.empty()) {
    	    ...
    	    if (it != cache.end()) {
    	      ...
    	      cache.insert({callee, CostBenefitInfo::LoopSet(stack.begin(), stack.end())});
    	    }
    	  }
    	  ...
    	  LoopInfo& li_callee =
    	    ...
    	    (*(const_cast<Function*>(callee))).getLoopInfo();
	  
    	  CBCG_LOG(errs() << "LOOP INFO FOR " << callee->getName() << ":\n";
    		   if (li_callee.empty())
    		     errs() << "NO LOOPS\n";
    		   else
    		     li_callee.print(errs()););

    	  check_loop_info_consistency(li_callee, callee, __LINE__);
    	  find_loops(callee->getEntryBlock(), li_callee, pass, stack, visited, cache, num_loops);
    	}
      }
    }
    
    // -- Continue recursively with the successors
    for (auto it = GT::child_begin(&cur), et = GT::child_end(&cur); it!=et; ++it) {
      NodeRef succ = *it;
      // recursive call
      ...
      LoopInfo& li_succ =
	pass->getAnalysis<LoopInfoWrapperPass>
	(*(const_cast<Function*>(succ->getParent()))).getLoopInfo();
  ...
  void CostBenefitCG::run(std::string benefit_filename,
			  ModulePass* pass,
    ...
    CBCG_LOG(// sanity check
	     ...
	     errs() << "Visited " << visited.size() << " blocks over " << total_bb << ".\n";
  ...
  CostBenefitCG::iterator CostBenefitCG::end()

...
public:
  ...
  typedef std::vector<CallGraphNodeWrapper *>::const_iterator
      const_succ_iterator;
...
class CallGraphWrapper {
  ...
  static const CallGraphNodeWrapper *getConstValue(const PairTy &p) {
    return &(p.second);
  }
  ...
  void addLinks(CallGraphNodeWrapper &n1, CallGraphNodeWrapper &n2) {
    if (std::find(n1._Children.begin(), n1._Children.end(), &n2) ==
        n1._Children.end())
...
public:
  CallGraphWrapper(CallGraph *CG) : _CG(CG) {
    ...
    for (CallGraph::iterator it = _CG->begin(), et = _CG->end(); it != et;
         ++it) {
    ...
    for (CallGraphNodeMapTy::iterator it = _CGM.begin(), et = _CGM.end();
         it != et; ++it) {
      CallGraphNode *N = it->first;
      for (CallGraphNode::iterator n_it = N->begin(), n_et = N->end();
           n_it != n_et; ++n_it) {
  ...
  typedef mapped_iterator<CallGraphNodeMapTy::iterator,
                          decltype(&CallGraphWrapper::getValue)>
      iterator;
  typedef mapped_iterator<CallGraphNodeMapTy::const_iterator,
                          decltype(&CallGraphWrapper::getConstValue)>
      const_iterator;
  ...
  iterator end() { return iterator(_CGM.end(), &CallGraphWrapper::getValue); }
  ...
  void print(raw_ostream &o) {
    ...
    while (!worklist.empty()) {
      ...
      Cur->print(o);
      o << "\n";
      for (CallGraphNodeWrapper::succ_iterator it = Cur->succ_begin(),
                                               et = Cur->succ_end();
           it != et; ++it) {
...
template <> struct GraphTraits<CallGraphNodeWrapper *> {
...
template <> struct GraphTraits<const CallGraphNodeWrapper *> {
...
template <> struct GraphTraits<Inverse<CallGraphNodeWrapper *>> {
  using NodeRef = CallGraphNodeWrapper *;
  using ChildIteratorType = CallGraphNodeWrapper::pred_iterator;

  static NodeRef getEntryNode(Inverse<CallGraphNodeWrapper *> G) {
    return G.Graph;
  }
...
template <> struct GraphTraits<Inverse<const CallGraphNodeWrapper *>> {
  using NodeRef = const CallGraphNodeWrapper *;
  using ChildIteratorType = CallGraphNodeWrapper::const_pred_iterator;

  static NodeRef getEntryNode(Inverse<const CallGraphNodeWrapper *> G) {
    return G.Graph;
  }
  static ChildIteratorType child_begin(NodeRef N) { return N->pred_begin(); }
  static ChildIteratorType child_end(NodeRef N) { return N->pred_end(); }
...
template <> struct GraphTraits<CallGraphWrapper *> {
  using nodes_iterator = CallGraphWrapper::iterator;
  using NodeRef = CallGraphNodeWrapper *;
  using ChildIteratorType = CallGraphNodeWrapper::succ_iterator;

  static NodeRef getEntryNode(CallGraphWrapper *CG) {
    return CG->getEntryNode();
  }
  static unsigned size(CallGraphWrapper *CG) { return CG->size(); }
  static nodes_iterator nodes_begin(CallGraphWrapper *CG) {
    return CG->begin();
  }
...
template <> struct GraphTraits<const CallGraphWrapper *> {
  using nodes_iterator = CallGraphWrapper::const_iterator;
  using NodeRef = const CallGraphNodeWrapper *;
  using ChildIteratorType = CallGraphNodeWrapper::const_succ_iterator;

  static NodeRef getEntryNode(const CallGraphWrapper *CG) {
    return CG->getEntryNode();
  }
  static unsigned size(const CallGraphWrapper *CG) { return CG->size(); }
  static nodes_iterator nodes_begin(const CallGraphWrapper *CG) {
    return CG->begin();
  }
  static nodes_iterator nodes_end(const CallGraphWrapper *CG) {
    return CG->end();
  }
  static ChildIteratorType child_begin(NodeRef N) { return N->succ_begin(); }
  static ChildIteratorType child_end(NodeRef N) { return N->succ_end(); }
